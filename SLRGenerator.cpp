...
// считываем грамматику и инициализируем стартовые пол€ таблицы
...
void SLRGenerator::FillTable()
{
	...
	while (true)
	{
		...
		// если очередь пуста€, то ищем элемент, дл€ которого будем добавл€ть строку
		if (q.empty())
		{
...
void SLRGenerator::AddStartingRule()
{
	...
	//  опирование элементов вектора во множество
	std::set<std::string> uniqueSet(firstFollowSet.begin(), firstFollowSet.end());

	//  опирование уникальных элементов обратно в вектор
